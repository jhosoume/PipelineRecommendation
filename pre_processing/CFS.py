...
for dataset in datasets:
    # dataset = "504_analcatdata_supreme.arff"
    name = dataset[:-5]
    print("[{}]".format(name))
    ...
    for train_indx, test_indx in divideFold.split(values):
        try:
            selected_features = PRE_PROCESSOR.cfs(values[train_indx], target[train_indx])
            selected_features = selected_features[0:values.shape[1]]
            new_values = values[:, selected_features][train_indx]
            new_target = target[train_indx]
