...
for dataset in models.name.unique():
    ...
    for indx, result in max_result.iterrows():
        if (result.preprocesses in ["None"] + constants.PRE_PROCESSES) and \
...
reg_models["knn"] = lambda: GridSearchCV(neighbors.KNeighborsRegressor(weights = "distance"), reg_models_params["knn"], cv = 5, n_jobs = -1, scoring = "neg_root_mean_squared_error", verbose = 2)
...
with open("analysis/plots/base_analysis/opt_" + SCORE + "normalized.json", "w") as fd:
