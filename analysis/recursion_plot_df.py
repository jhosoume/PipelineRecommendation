...
PATH = "analysis/plots/recursion/default/"
...
mean_res = {type_res: list(np.sum([res[type_res]["default"]
                for res in results], axis = 0)/len(results))
                for type_res in results[0].keys()}
std_res = {type_res: list(np.std([res[type_res]["default"]
                for res in results], axis = 0)/len(results))
                for type_res in results[0].keys()}
mean_totes = list(np.sum([tot["default"]
                for tot in totals], axis = 0)/len(totals))
std_totes = list(np.std([tot["default"]
                for tot in totals], axis = 0)/len(totals))
...
fig.write_image("analysis/plots/recursion/df.eps")
fig.write_image("analysis/plots/recursion/df.png")
fig.write_image("/home/jhosoume/unb/tcc/ICDM/img/recursion_analysis/rec.df.eps")
fig.write_image("/home/jhosoume/unb/tcc/ICDM/img/recursion_analysis/rec.df.png")
