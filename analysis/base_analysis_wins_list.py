    ...
    for dataset in database.name.unique():
        ...
        for clf in constants.CLASSIFIERS:
            for pp in constants.PRE_PROCESSES + ['None']:
                if len(split.query("classifier == '{}' and preprocesses == '{}'".format(clf, pp))) < 1:
                    keep = False
        if keep:
...
for rep in range(REP):
    ...
    for baseline in ["default", "random"]:
        ...
        for regressor_type in filter(lambda reg: not reg in ["random", "default"], constants.REGRESSORS):
            ...
            kfold = 0
            results[baseline][regressor_type] = 0
            total_datasets[baseline][regressor_type] = 0
            draws[baseline][regressor_type] = 0
            ...
            for train_indx, test_indx in divideFold.split(datasets):
                ...
                for test_dataset in tests.name.unique():
                    ...
                    # Storing the result
                    if float(score_pred) > float(score_baseline):
                        results[baseline][regressor_type] += 1
                    elif float(score_pred) == float(score_baseline):
                        draws[baseline][regressor_type] += 1
                    total_datasets[baseline][regressor_type] += 1
    ...
    with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep" + str(REP) + "_res_" + str(rep) + "_wins.json", "w") as fd:
        json.dump(results, fd, indent = 4)
    with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep" + str(REP) + "_res_" + str(rep) + "_total.json", "w") as fd:
        json.dump(total_datasets, fd, indent = 4)
    with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep" + str(REP) + "_res_" + str(rep) + "_draws.json", "w") as fd:
        json.dump(draws, fd, indent = 4)

    non_normalized_results = results.copy()

    all_results.append(results)
    all_draws.append(draws)
    all_totals.append(total_datasets)

with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep_" + str(REP) + "_wins.json", "w") as fd:
    json.dump(all_results, fd, indent = 4)
with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep_" + str(REP) + "_draws.json", "w") as fd:
    json.dump(all_draws, fd, indent = 4)
with open("analysis/plots/base_analysis/" + SCORE + "_normalized_rep_" + str(REP) + "_totals.json", "w") as fd:
    json.dump(all_totals, fd, indent = 4)
