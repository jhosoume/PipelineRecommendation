...
# Geting packages
packages = ("reshape2", "e1071", "kknn", "randomForest")
...
from rpy2.robjects.packages import SignatureTranslatedAnonymousPackage
regressors_R_string = """
ANN <- function(tec, tran, test) {
    mlp <- RWeka::make_Weka_classifier("weka/classifiers/functions/MultilayerPerceptron")
    model <- mlp(tec, tran)
    predict(model, test)
}

CART <- function(tec, tran, test) {
    model <- rpart::rpart(tec, tran)
    as.numeric(predict(model, test))
}

DWNN <- function(tec, tran, test) {
    model <- kknn::kknn(tec, tran, test, kernel="gaussian")
    model$fitted.values
}

RFR <- function(tec, tran, test) {
    model <- randomForest::randomForest(tec, tran)
    as.numeric(predict(model, test))
}

SVR <- function(tec, tran, test) {
    model <- e1071::svm(tec, tran, scale=TRUE, type="eps-regression", kernel="radial")
    as.numeric(predict(model, test))
}
...
"""
R_regressors = SignatureTranslatedAnonymousPackage(regressors_R_string, "R_regressors")
